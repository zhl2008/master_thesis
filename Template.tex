...
随着各类基于大数据和AI的应用兴起，快速廉价地获取大量有效数据的能力，成为互联网时代企业和个人竞争力的体现。因此，网络爬虫在数据收集收集方面的重要性逐渐凸显出来。但是，恶意爬虫同样给互联网用户和互联网服务提供者带来了巨大的困扰，这些爬虫或者多线程、高并发的请求消耗服务器的带宽和计算资源，或者爬取个人敏感信息、高价值的商业数据用于不法用途。
 ...
 \begin{itemize}
\item[1）] 提出了一种充分利用浏览器静态信息和动态指纹信息的session分类方法，即使请求数据中存在对抗样本时，也能保证session分类的准确率；
...
\item[4）] 结合上述对爬虫检测技术与爬虫对抗技术的研究，实现了一套具有一定完备性和健壮性的反爬虫系统原型（Crawler-Killer)，并实验论证了该系统的有效性，最终通过该系统发现了某大型搜索引擎厂商的一枚高危爬虫漏洞。
 \end{itemize}
  }{%
...
截止到本文提交前，该攻击载荷还成功攻击了来自于大型搜索引擎厂商的爬虫，成功实现了任意文件读取的攻击效果，并因为可能会泄露爬虫算法和SEO规则，被评级为核心业务的高危漏洞，如图\ref{fig:crawler-loophole}所示。具体攻击细节会在其修复相关漏洞后公布。


\begin{figure}[!h]
  \centering
  \includegraphics[width=0.68\textwidth]{images/crawler_loophole.png}
  \caption{某大型搜索引擎厂商的爬虫漏洞确认}
...
\subsection{爬虫对抗效果}
我们通过手动降低恶意爬虫识别的阈值的方法，将所有类型的爬虫所在的session以及IP列入黑名单，并启用所有的攻击载荷。一段时间后，部分爬虫因为执行了资源消耗型的攻击载荷，向服务器发送的请求速度显著降低。部分爬虫因为执行了信息窃取型的攻击载荷，会将自身内网相关的敏感信息泄露到到了指定的远程服务器上。甚至部分使用PhantomJS作为webdriver的爬虫，因为触发了PhantomJS的内存破坏漏洞的攻击载荷，爬虫进程直接奔溃退出，如图\ref{fig:res}所示。具体的攻击效果会作为demon演示。
